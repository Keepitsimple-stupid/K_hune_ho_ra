"""
Multi-domain analysis agent manager for the Neural News Analysis System.

This module manages 15 specialized reasoning agents that analyze news articles
from different domain perspectives (financial, geopolitical, technological, etc.)
using a local LLM to generate detailed predictions.
"""
...
class AgentManager:
    """
    Manages and executes multiple domain-specific analysis agents.
    
    Each agent is configured with a specialized prompt to analyze news articles
    from a particular domain perspective, generating 5-10 detailed predictions
    with confidence scores and timelines.
    """
    def __init__(self):
        """
        Initialize the agent manager and load the LLM model.
        
        Loads the Qwen2.5-7B model with GPU acceleration and configures
        domain-specific prompts for each analysis agent.
        
        Raises:
            SystemExit: If model loading fails, with troubleshooting guidance
        """
        ...
        # Each prompt instructs the LLM to generate 5-10 detailed predictions
        # with standardized JSON output including prediction, confidence, timeline,
...
No extra text outside JSON."""
    ...
    def _truncate_articles(self, articles, max_chars=6000):
        """
        Truncate article content to fit within LLM context window.
        
        Concatenates article titles, dates, and snippets into a single string,
        then truncates to the specified maximum character limit.
        
        Args:
            articles: List of article dictionaries
            max_chars: Maximum characters to include in context (default: 6000)
            
        Returns:
            Truncated article context string
        """
    ...
    def analyze_domain(self, domain: str, articles: list) -> dict:
        """
        Run a single domain analysis agent on the provided articles.
        
        Constructs a domain-specific prompt with article context, queries the LLM,
        and parses the JSON response into a structured predictions list.
        
        Args:
            domain: The domain name (e.g., 'financial', 'geopolitical')
            articles: List of article dictionaries to analyze
            
        Returns:
            Dictionary with 'predictions' list or 'error' field if parsing fails
        """
...
[
  ...
  ...
        ...
        response = self.llm(
            prompt,
            max_tokens=config.MAX_TOKENS * 2,  # Allow more tokens for multiple predictions
    ...
    def run_all_agents(self, articles: list) -> dict:
        """
        Execute all 15 domain analysis agents sequentially.
        
        Iterates through all configured domains, runs each agent on the
        provided articles, and collects the results. Progress is printed
        to the console with prediction counts.
